...
routerAdd('POST', '/api/crawl/login', (e) => {
  ...
  // 20/15min protects the one shared crew password from one IP in production. A test harness runs
  // the whole e2e suite from a single loopback address, so it can raise this via the environment
  // (only `web/playwright.config.ts` does); anyone who does not set it gets the production default.
