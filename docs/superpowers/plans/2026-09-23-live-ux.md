...
```
Lightbox test ids: `lightbox`, `lightbox-close`. The viewer is a native `<dialog>` opened with `showModal()` so it stacks above the stop sheet's modal (Task 5); assertions use `toBeVisible()`/`toBeHidden()`, since the element is always mounted. Freight thumbnails become `<button data-testid="freight-open-{i}">`.
...
**Interfaces:**
- Consumes: `liveDay.feed`, `liveDay.loadFeed`, `liveDay.upsertDrink`, `liveDay.dropDrink` (Task 2); `drinkCount` from `$lib/live/crew`; `newRecordId` from `$lib/live/staged`.
...
```
`TabRow` props become `{ entries: DrinkEntry[]; stopId: string | null; userId: string; total: number; clinking: DrinkKind | null; onlog: (kind: DrinkKind) => void; onundo: (entry: DrinkEntry) => void }`, where `entries` already include pending taps. Test ids kept: `drink-{kind}` (with a `.count` child), `tab-undo` (the toast's Undo); new: `tab-total`, `tab-toast`, `tab-undo-last` (fallback undo for your newest saved entry at this stop, shown when no toast is up). Removed: `action-tab`, `tab-dialog`, `close-tab`.

**Why ids:** a tap creates its row with a client-generated id (the same `newRecordId()` Bulletins use). The pending placeholder and the saved row share that id, so a realtime reload that lands before the create response cannot count the drink twice, and a successful create is written into the feed at once (`upsertDrink`), so a later failed or stale feed read cannot make it disappear.
...
```ts
import { withPending } from '../../src/lib/live/tab';
...
```
(`DrinkEntry` is already imported in that file; add the import if not.)
...
`tab.spec.ts`, first test: delete `await page.getByTestId('action-tab').click();`; after `await expect(beer).toContainText('1');` add `await expect(page.getByTestId('tab-total')).toContainText('1');`. Second test is unchanged. Add `import { copy } from '../../src/lib/labels';` and append:
...
Add to `copy`: `tabYou: 'you',`, `tabAdded: '+1',`, `undoLast: 'Undo my last one here',`.
...
1. Imports: remove the `tabDialog`, `tabOpen` state; import `withPending` from `$lib/live/tab`, `drinkCount` from `$lib/live/crew`, `todayInTz` from `$lib/time`, `drinkIcons` from `$lib/labels`, `newRecordId` from `$lib/live/staged`.
...
The leaderboard (Task 10) and the chat keep reading `liveDay.feed.drinks`; only the Tab row shows pending taps.
