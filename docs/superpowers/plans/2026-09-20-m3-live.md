...
One request that validates, writes and logs. The only way a locked route changes.

**Files:**
- Create: `web/src/routes/api/plan/commit/+server.ts`
- Modify: `docs/superpowers/specs/2026-09-20-m3-live-design.md` (§3.4 write order)
...
- Produces: `POST /api/plan/commit` with body
  ...
  {
    itinerary: string;
    anchorStopId: string;               // may be a tmp_ id of a stop created in this same commit
    stops: Array<{ id: string; order: number; name: string; kind: string; station_id: string;
      station_name: string; dwell_min: number; walk_min: number; direction?: string;
      place?: string; place_id?: string; osm_id?: string; address?: string; lat?: number; lon?: number;
      phone?: string; website?: string }>;
    removed: string[];
    bulletin?: { kind: BulletinKind; body: string } | null;
  }
  ```
  answering `200 {{ ok: true, anchorAt, legs, broadcast }}` or `409 {{ ok: false, blockers }}`.

**Write order** (and why): stops first, then the anchor, because the crew may be standing in a stop this very commit creates and the anchor needs its real id. Then recompute, then the Bulletin, then the Train Sheet. PocketBase has no cross-request transaction, so the order is chosen to be safe to re-drive rather than atomic.

- [ ] **Step 1: Correct the spec's write order**

In `docs/superpowers/specs/2026-09-20-m3-live-design.md` §3.4, swap items 2 and 3 so the list reads: re-run the preview; apply stop deletes, updates and creates; create the anchor `checkins` row with `at` = the server's now (the crew may be standing in a stop this commit just created, so the anchor needs its real id); recompute; broadcast; log.
...
vi.mock('$lib/server/pb', () => ({
  ...
  adminPb: vi.fn(async () => ({
    ...
    collection: (collection: string) => ({
      getOne: async () => state.itinerary,
      create: async (body: Record<string, unknown>) => {
        const id = `new${++state.nextId}`;
        state.writes.push({ op: 'create', collection, body });
        return { id, ...body };
      },
      update: async (id: string, body: Record<string, unknown>) => { state.writes.push({ op: 'update', collection, id, body }); return { id }; },
      delete: async (id: string) => { state.writes.push({ op: 'delete', collection, id }); }
    })
  }))
}));
vi.mock('$lib/server/metra', () => ({ metra: { getSchedule: vi.fn(async () => fixtureSchedule()) } }));
...
const rideable = {
  itinerary: 'it1', anchorStopId: 's2', removed: [],
  stops: [
...
beforeEach(() => {
  ...
  state.writes = [];
  state.nextId = 0;
  vi.mocked(recomputeItinerary).mockClear();
...
describe('POST /api/plan/commit', () => {
  ...
  it('applies the stops, then anchors the crawl, then recomputes', async () => {
    const res = await call({
      ...rideable,
      removed: ['sX'],
      stops: [...rideable.stops, { id: 'tmp_1', order: 3, name: 'Prairie Path Tap', kind: 'bar', station_id: 'CUS', station_name: 'Union Station', dwell_min: 45, walk_min: 6 }]
    });
    expect(res.status).toBe(200);

    const order = state.writes.map((w) => `${w.op}:${w.collection}`);
    expect(order.slice(0, 2)).toEqual(['delete:stops', 'update:stops']);
    expect(order).toContain('create:stops');
    ...
    expect(recomputeItinerary).toHaveBeenCalledWith('it1');

    const anchor = state.writes.find((w) => w.collection === 'checkins')!;
    expect(anchor.body).toMatchObject({ user: 'u1', stop: 's2', kind: 'at_stop', at: '2026-12-26T18:00:00.000Z' });
  });

  it('anchors on a stop this very commit created', async () => {
    const res = await call({
      itinerary: 'it1', anchorStopId: 'tmp_1', removed: [],
      stops: [
        { id: 'tmp_1', order: 1, name: 'Prairie Path Tap', kind: 'bar', station_id: 'LAGRANGE', station_name: 'La Grange Road', dwell_min: 60, walk_min: 5 },
        rideable.stops[1]
      ]
    });
    expect(res.status).toBe(200);
    expect(state.writes.find((w) => w.collection === 'checkins')!.body).toMatchObject({ stop: 'new1' });
  });

  it('posts the Bulletin when one is kept, and reports its id', async () => {
    const res = await call({ ...rideable, bulletin: { kind: 'hold', body: 'Holding at The Hop Haus.' } });
    const body = await res.json();
    expect(body.broadcast).toBeTruthy();
    const bulletin = state.writes.find((w) => w.collection === 'broadcasts')!;
    expect(bulletin.body).toMatchObject({ itinerary: 'it1', kind: 'hold', body: 'Holding at The Hop Haus.', created_by: 'u1' });
  });
  ...
  it('refuses to edit an itinerary that is not locked', async () => {
    state.itinerary = { id: 'it1', status: 'draft', event_date: '2026-12-26', start_time: '11:00' };
...
type CommitStop = {
  id: string; order: number; name: string; kind: string; station_id: string; station_name: string;
  dwell_min: number; walk_min: number; direction?: string; place?: string; place_id?: string; osm_id?: string;
  address?: string; lat?: number; lon?: number; phone?: string; website?: string;
};
type CommitBody = {
...
/** A stop the editor invented: it has no record yet, so the commit creates it. */
const isNew = (id: string) => id.startsWith('tmp_');

// The only write path for a locked route. Validate, apply, anchor, recompute, tell the crew, log.
// Not a transaction — PocketBase has none across requests — so the order is the one that is safe to
// re-drive after a failure: stops are idempotent against the submitted plan, the anchor is harmless
// on its own, and recompute is idempotent by construction.
export const POST: RequestHandler = async ({ request }) => {
  ...
  // 1. Stops. Deletes first so a freed `order` cannot collide with an update.
  for (const stopId of removed) await pb.collection('stops').delete(stopId);
  const ids = new Map<string, string>();
  for (const s of stops.filter((x) => !isNew(x.id))) {
    await pb.collection('stops').update(s.id, {
  ...
  for (const s of stops.filter((x) => isNew(x.id))) {
    const created = await pb.collection('stops').create({
      itinerary: id, order: s.order, name: s.name, kind: s.kind, station_id: s.station_id, station_name: s.station_name,
      dwell_min: s.dwell_min, walk_min: s.walk_min, direction: s.direction ?? '', place: s.place ?? '',
      place_id: s.place_id ?? '', osm_id: s.osm_id ?? '', address: s.address ?? '', lat: s.lat ?? 0, lon: s.lon ?? 0,
      phone: s.phone ?? '', website: s.website ?? ''
    });
    ids.set(s.id, created.id);
  }

  // 2. The anchor, now that every stop it could point at exists.
  const anchorId = ids.get(anchorStopId) ?? anchorStopId;
  await pb.collection('checkins').create({ user: user.id, stop: anchorId, kind: 'at_stop', at });

  // 3. The legs everyone reads. The stops hooks fire their own recomputes; the per-itinerary queue
  ...
  // 4. The Bulletin, if the Conductor kept it.
  let broadcast: string | null = null;
  if (body.bulletin?.body) {
    const created = await pb.collection('broadcasts').create({
      itinerary: id, kind: body.bulletin.kind, body: body.bulletin.body, created_by: user.id
    });
  ...
  await pb.collection('event_log').create({
    ...
    payload: { stops: stops.length, added: [...ids].filter(([k]) => isNew(k)).length, removed: removed.length, anchor: anchorId, broadcast }
  });

  return json({ ok: true, anchorAt: at, legs: recomputed.legs, broadcast });
};
```
...
```bash
git add web/src/routes/api/plan/commit web/tests/unit/planCommit.test.ts docs/superpowers/specs/2026-09-20-m3-live-design.md
git commit -m "feat(api): commit a live plan change in one coherent write"
```
...
The editor's model: a plan held in the browser, changed by pure functions, turned into a commit payload at the end.
...
- Produces:
  ...
  export const isNewStop: (id: string) => boolean;
  export type StagedStop = { id: string; order: number; name: string; kind: VenueKind; station_id: string;
    station_name: string; dwell_min: number; walk_min: number; direction: 'out' | 'back' | '';
  ...
  export function addStop(plan: StagedPlan, stop: Omit<StagedStop, 'id' | 'order'>, index: number): StagedPlan;
...
```ts
import { describe, expect, it } from 'vitest';
import { addStop, commitPayload, isNewStop, moveStop, removeStop, setAnchor, setDwell, stagePlan } from '../../src/lib/live/staged';
...
describe('editing', () => {
  ...
  it('forgets a staged stop instead of queueing a delete for a record that never existed', () => {
    const added = addStop(base(), { name: 'Prairie Path Tap', kind: 'bar', station_id: 'CUS', station_name: 'Union Station', dwell_min: 45, walk_min: 6, direction: 'out' }, 3);
    const gone = removeStop(added, added.stops[3].id);
    expect(gone.removed).toEqual([]);
    expect(gone.stops).toHaveLength(3);
  });

  it('inserts a new stop with a temporary id', () => {
    const plan = addStop(base(), { name: 'Prairie Path Tap', kind: 'bar', station_id: 'CUS', station_name: 'Union Station', dwell_min: 45, walk_min: 6, direction: 'out' }, 1);
    expect(plan.stops.map((s) => s.name)).toEqual(['The Hop Haus', 'Prairie Path Tap', 'The Second Round', 'Berwyn Beer Hall']);
    expect(isNewStop(plan.stops[1].id)).toBe(true);
    expect(plan.stops.map((s) => s.order)).toEqual([1, 2, 3, 4]);
  });

  it('gives every new stop its own id', () => {
    const one = addStop(base(), { name: 'A', kind: 'bar', station_id: 'CUS', station_name: 'Union Station', dwell_min: 30, walk_min: 5, direction: 'out' }, 3);
    const two = addStop(one, { name: 'B', kind: 'bar', station_id: 'CUS', station_name: 'Union Station', dwell_min: 30, walk_min: 5, direction: 'out' }, 4);
...
describe('commitPayload', () => {
  it('carries the itinerary, the anchor, the stops and the deletions', () => {
    ...
    expect(commitPayload(plan, 'it1')).toEqual({
      itinerary: 'it1', anchorStopId: 'b', removed: ['c'], stops: plan.stops
    });
  });
});
```
...
export const TEMP_PREFIX = 'tmp_';
/** A stop the Conductor invented in this editing session: no record exists for it yet. */
export const isNewStop = (id: string): boolean => id.startsWith(TEMP_PREFIX);

export type StagedStop = {
  id: string; order: number; name: string; kind: VenueKind; station_id: string; station_name: string;
  dwell_min: number; walk_min: number; direction: 'out' | 'back' | '';
  /** Venue details carried through to the create call for a brand-new stop. */
  extra?: Record<string, unknown>;
};
export type StagedPlan = { stops: StagedStop[]; anchorStopId: string | null; removed: string[] };
...
export function removeStop(plan: StagedPlan, stopId: string): StagedPlan {
  if (!plan.stops.some((s) => s.id === stopId)) return plan;
  return {
    ...plan,
    ...
    removed: isNewStop(stopId) ? plan.removed : [...plan.removed, stopId]
  };
}

export function addStop(plan: StagedPlan, stop: Omit<StagedStop, 'id' | 'order'>, index: number): StagedPlan {
  const used = plan.stops.filter((s) => isNewStop(s.id)).map((s) => Number(s.id.slice(TEMP_PREFIX.length)) || 0);
  const id = `${TEMP_PREFIX}${Math.max(0, ...used) + 1}`;
  const next = sorted(plan.stops);
  next.splice(Math.max(0, Math.min(index, next.length)), 0, { ...stop, id, order: 0 });
  return { ...plan, stops: renumber(next) };
}
...
git commit -m "feat(live): the staged plan the live editor holds until Save"
```
...
describe('recordActions', () => {
  ...
  it('reports a failure through onerror instead of throwing', async () => {
    ...
    const seen: string[] = [];
    await recordActions('it1', (m) => seen.push(m)).setDwell('a', 30);
...
describe('previewPlan', () => {
  it('sends only what the planner needs', async () => {
    await previewPlan(plan, 'it1');
    expect(sent.path).toBe('/api/plan/preview');
    expect(sent.body).toEqual({
      itinerary: 'it1', anchorStopId: 'a',
      stops: [
  ...
  it('renames the fields so the itinerary components can render it', async () => {
    const legs = await previewPlan(plan, 'it1');
    expect(legs[0]).toMatchObject({
      id: 'preview:a:b', itinerary: 'it1', from_stop: 'a', to_stop: 'b', kind: 'train',
...
- Produces: the editor's staged mode; `sessionStorage` key `chugalug.stagedAdd:<itineraryId>` carrying a venue
  picked on the add screen back to the editor as `{ name, kind, station_id, station_name, dwell_min, walk_min, direction, place, place_id, osm_id, address, lat, lon, phone, website, index }`.
...
<script lang="ts">
  ...
  import { addStop, commitPayload, moveStop, removeStop, setAnchor, setDwell, stagePlan, type StagedPlan } from '$lib/live/staged';
  import { previewPlan } from '$lib/live/preview';
  import { cohesionBlockers } from '$lib/live/cohesion';
  import ItineraryView from '$lib/components/ItineraryView.svelte';
  import type { Checkin, Leg } from '$lib/types';
  ...
  async function load() {
    try {
      ...
      if (draft.itinerary.status === 'locked' && !plan) plan = stagePlan(draft.stops, await loadAnchor());
  ...
  $effect(() => {
    draft = null; error = ''; plan = null; previewLegs = [];
    void load();
  ...
  const stagedActions: PlanActions = {
    ...
    add: (stationId, side) => {
      try { sessionStorage.setItem(`chugalug.stagedAddSide:${data.id}`, side); } catch { /* private mode */ }
  ...
  // A venue picked on the add screen while The Route is locked comes back through sessionStorage.
  $effect(() => {
    if (!plan) return;
    ...
    if (!raw) return;
    try {
      const { index, ...stop } = JSON.parse(raw);
      plan = addStop(plan, stop, Number(index) || plan.stops.length);
    } catch { /* a corrupt handoff is not worth an error message */ }
  });
...
```ts
    ...
    if (page.url.searchParams.get('staged') === '1') {
      const at = insertionIndex(stations, [], station.id, direction);
      const payload = {
        index: at, name: venue.name, kind: venue.kind, station_id: station.id, station_name: station.name,
...
`insertionIndex` is called with an empty list because the staged plan, not the database, decides the slot; the editor inserts at that station-derived index and renumbers. (The editor's `addStop` clamps the index, so an out-of-range value is harmless.)
...
In the edit screen, keep the plan that was loaded so the diff has a "before":

```ts
  let before = $state<PlanSnapshot | null>(null);   // set alongside `plan` in load()
  let pending = $state<{ text: string; kind: BulletinKind } | null>(null);
```

In `load()`, after `plan = stagePlan(...)`, add `before = snapshot(plan)`, where
...
```ts
  function askToTell() {
    ...
    pending = { text: bulletinText(changes), kind: bulletinKind(changes) };
  }

  async function commit(bulletin: { kind: BulletinKind; body: string } | null) {
    if (!plan) return;
    pending = null; saving = true; error = '';
    try {
      await api('/api/plan/commit', { method: 'POST', json: { ...commitPayload(plan, data.id), bulletin } });
      await goto(`/plan/${data.id}`);
    } catch (err) {
      error = (err as Error).message || copy.saveFailed;
...
{#if pending}
  <BulletinSheet text={pending.text}
    onsend={(body) => commit({ kind: pending!.kind, body })}
    onskip={() => commit(null)} />
{/if}
```
...
```ts
  async function postBulletin(body: string) {
    ...
    try {
      await pb.collection('broadcasts').create({ itinerary: liveDay.itinerary.id, kind: 'message', body, created_by: $auth.user.id });
      await liveDay.loadBulletins();
...
const itinerary = { id: 'it1', title: 'The Route', status: 'locked', event_date: '2026-12-26' } as Itinerary;
