...
const rideable = {
  itinerary: 'it1', anchorStopId: 's2', removed: [] as string[],
  stops: [
...
describe('POST /api/plan/commit', () => {
  ...
  it('applies the stops, then anchors the crawl, then recomputes', async () => {
    ...
    expect(state.writes.find((w) => w.op === 'create' && w.collection === 'stops')!.body)
      .toMatchObject({ id: 'abcdefghij01234', itinerary: 'it1', name: 'Prairie Path Tap' });
...
- Produces:
  ...
  export type StagedStop = { id: string; isNew?: boolean; order: number; name: string; kind: VenueKind;
    ...
    direction: 'out' | 'back' | ''; extra?: Record<string, unknown> };
...
export type StagedStop = {
  id: string;
  /** Staged in this session: the commit creates it rather than updating it. */
  isNew?: boolean;
