...
- **Plan time**: wall time on the event day. On a practice day it is
  `projectToPlanDate(wall, event_date)`, meaning `localToUtc(event_date, minutesOfDay(todayInTz(wall), wall))`.
  It is used only for:
  - `currentStop()`
...
- `liveDay` reloads the feed when the Chicago date changes (checked on its tick). A client open over
  midnight then empties without a reload.
...
- **How it is created:** through the normal path, so the hooks run:
  ...
  2. create the stops
  3. attach places and photos
  4. lock the route by update
  5. wait for the recompute to produce legs
  6. check that every leg is `train` or `walk`
  7. set `crawl_settings.current_itinerary` if it is empty
- If a leg is impossible, the script fails loudly and leaves the draft for inspection.
