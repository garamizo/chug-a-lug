...
const state = vi.hoisted(() => ({
  ...
  itinerary: { id: 'it1', status: 'locked', event_date: '2026-12-26', start_time: '11:00' } as Record<string, unknown>
}));
...
beforeEach(() => {
  ...
  state.itinerary = { id: 'it1', status: 'locked', event_date: '2026-12-26', start_time: '11:00' };
});
...
describe('POST /api/plan/preview', () => {
  it('plans the submitted stops from the anchor moment', async () => {
    ...
    const res = await call({ itinerary: 'it1', anchorStopId: 's2', stops });
  ...
  it('plans from the itinerary start time with no anchor', async () => {
    const res = await call({ itinerary: 'it1', stops });
    const body = await res.json();
  ...
  it('refuses a stop list that is not shaped like stops', async () => {
    await expect(call({ itinerary: 'it1', stops: [{ id: 's1' }] })).rejects.toMatchObject({ status: 400 });
  });
});
