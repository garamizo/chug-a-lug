...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    if (context.enabled && body.clockRevision !== context.revision) throw new ClockServiceError(409, 'simClockConflict', context);
    ...
    const at = context.eventNow;
    const anchor = activeAnchor(itinerary.event_date, anchorStopId ? { stopId: anchorStopId, at } : null, new Date(at));
    ...
    return await simulationClock.withEventWrite(context.enabled ? context.revision : undefined, async () => {
      ...
      const recomputed = await recomputeItinerary(id, context);
